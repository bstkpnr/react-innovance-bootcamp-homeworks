    ...
    axios.get(API_URL).then((res) => {
      res.data.forEach((element) => {
        element.languages.forEach((lang) => {
          let findIndex = newLangDatas.findIndex(
            (lng) => lng.iso639_1 === lang.iso639_1
          );
          if (findIndex === -1) {
      ...
      <div className="container  d-flex justify-content-center my-3">
        <Card className="shadow-lg  bg-white rounded">
          <ListGroup className="list-group-flush p-4 col-12 ">
            <h4 className="p-3 mb-2 bg-secondary text-white rounded">
              Language Top 10
            </h4>
            {languages.map(
              (lang, index) =>
                index < 10 && (
                  <p>
                    <span className="badge bg-dark p-3 col-12">
